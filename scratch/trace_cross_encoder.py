...
def analyze_case(case_id):
    ...
    for candidate in candidates:
        ...
        # We do the exact confidence and score math as in match_schemes
        ...
        # Filter like match_schemes top_matches
    ...
    for i, item in enumerate(ce_ranked):
        print(f"  Rank {i+1}: {item['scheme_id']} - Name: {item['name']} - CE Score: {item['ce_score']:.4f} - Retr: {item['retrieval_score']} - Conf: {item['confidence']}")

analyze_case("health_004")
analyze_case("fin_002")
